...
function evaluateLocationAvailability(meta, timePhase) {
  ...
  switch (mode) {
    ...
    case "restricted":
      available = false;
      if (!reason) {
        reason = availability.restrictedProfile === "old-city-epic-lock" ? "Old City access unlocks in a later chapter." : "Currently restricted.";
      }
      break;
    case "always":
    default:
...
function getLocationAvailability(state, { districtId = null, poiId = null } = {}) {
  ...
  const entries = [district, pointOfInterest].filter((entry) => Boolean(entry));
